...
**未做**（诚实标注，[需确认]，按推荐默认继续，不阻塞后续 Phase）：
- 未做全量 74212 pair 的子图抽取（性能瓶颈，见 §2）。
- 未跑满 1 epoch，未跑 Llama-3-8B 论文模型规模。
- 未做与 Phase 0 基线的下游生成质量对比（`evaluation/eval_lite.py` 全指标）——
  在只有 6.7% 样本有真实子图信号、且未训练到收敛的情况下，跑评估没有信息量，
  跑出来的数字不能代表"子图注入是否真的提升解释质量"。
- **推荐**：若要拿到有意义的质量对比，需要先解决 §2 的抽取性能问题（或至少扩大到更大比例的
  训练集），再跑一次真正意义上的全量/近全量训练 + `eval_lite.py` 对比。这笔投入应该放在
  确认 Phase 2 的 LightGCN backbone 换血（更大预期收益、更低成本）之后。
...
**部分通过**：soft-prompt 注入机制端到端验证通过（真实训练 loss 下降、无崩溃），这是 Phase 3
的核心技术风险点，已经排除。受限于子图抽取性能瓶颈，**规模验证**（全量数据、收敛、下游质量
对比）未完成，诚实记录为后续待办，不阻塞进入 Phase 4。
